...
type SliderImage = {
  src: string;
  alt: string;
  iconSrc: string; // Changé de icon à iconSrc pour une URL d'image
  tagLink: string;
};
...
const ExpandSlider = ({ images, className }: ExpandSliderProps) => {
  ...
  return (
    <div className={cn("w-full flex flex-col gap-4 expand-slider-container hidden md:block", className)}>
      <div className="flex w-full h-[500px] md:h-[600px] gap-2 overflow-hidden rounded-2xl">
        {images.map((image, index) => (
          ...
          >
            ...
            <div className={cn(
              ...
              <h3 className="flex items-center gap-2 text-white font-semibold text-xl tracking-wide">
                <img 
                  src={image.iconSrc} 
                  alt={`${image.alt} icon`}
                  className="w-6 h-6"
